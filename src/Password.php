...
/**
 * @file
 * Most of this file is copied from Drupal 7's includes/password.inc.
 * drupal_random_bytes() is copied from bootstrap.inc.
 * This lets us hash passwords just like D7 before sending them over services.
 * NB that this assumes the following variables in Drupal:
...
class Password {
   ...
   *   Encoded string
   */
  public static function _password_base64_encode($input, $count) {
    $output = '';
    $i = 0;
   ...
   */
  public static function _password_generate_salt($count_log2) {
    $output = '$S$';
   ...
   */
  public static function _password_enforce_log2_boundaries($count_log2) {
   ...
   */
  public static function _password_crypt($algo, $password, $setting) {
   ...
   */
  public static function _password_get_count_log2($setting) {
    $itoa64 = self::_password_itoa64();
   ...
   */
  public static function user_hash_password($password, $count_log2 = 0) {
    if (empty($count_log2)) {
      // Use the standard iteration count.
      $count_log2 = self::DRUPAL_HASH_COUNT;
    }
   ...
   *   TRUE or FALSE.
   */
  public static function user_check_password($password, $account) {
   ...
   *
   * Alternative implementations of this public static function might use other criteria based
   ...
   *   TRUE or FALSE.
   */
  public static function user_needs_new_hash($account) {
    // Check whether this was an updated password.
    if ((substr($account->pass, 0, 3) != '$S$') || (strlen($account->pass) != self::DRUPAL_HASH_LENGTH)) {
      return TRUE;
    }
    // Ensure that $count_log2 is within set bounds.
    $count_log2 = self::_password_enforce_log2_boundaries(self::DRUPAL_HASH_COUNT);
    // Check whether the iteration count used differs from the standard number.
    return (self::_password_get_count_log2($account->pass) !== $count_log2);
  }
   ...
   */
  public static function drupal_random_bytes($count)  {
